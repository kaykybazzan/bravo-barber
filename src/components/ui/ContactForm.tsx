              ...
              <Field id={`nx-${field.name}`} label={field.label} required={field.required} error={errors[field.name]} hint={field.hint}>
                {({ id, describedBy, invalid }) => {
                  if (field.type === "textarea") {
                    return <textarea id={id} name={field.name} rows={5} value={value} onChange={(e) => update(field.name, e.target.value)} aria-describedby={describedBy} aria-invalid={invalid} className={cn(controlClassFor(invalid), "resize-y")} placeholder={field.placeholder} maxLength={field.maxLength} />;
                  }
                  if (field.type === "select") {
                    return (
                      <select id={id} name={field.name} value={value} onChange={(e) => update(field.name, e.target.value)} aria-describedby={describedBy} aria-invalid={invalid} className={controlClassFor(invalid)}>
