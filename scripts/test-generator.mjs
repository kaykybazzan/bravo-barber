...
try {
  ...
  const sourceServices = readFileSync(join(ROOT, "src/data/services.ts"), "utf8");
  const sourceIsDemo = sourceServices.includes("DEMO DATA") || sourceServices.includes("diagnostico-eletronico");
  ...
  if (sourceIsDemo) {
    ...
    assert(services === sourceServices, "conteúdo personalizado do cliente foi sobrescrito pelo gerador");
  }
